...
function loadSudoUsers() {
    try {
        if (!fs.existsSync(SUDO_FILE)) return { users: [] };
        const parsed = JSON.parse(fs.readFileSync(SUDO_FILE, 'utf8'));
        const users = Array.isArray(parsed.users) ? parsed.users : [];
        return { users: [...new Set(users.map(normalizeUserId).filter(Boolean))] };
    } catch (error) {
...
function saveSudoUsers(sudoUsers) {
    try {
        ...
        const users = [...new Set((sudoUsers.users || []).map(normalizeUserId).filter(Boolean))];
